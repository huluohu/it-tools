...
<template>
  <div flex flex-col justify-center gap-2>
    <div flex items-center>
      <label w-75px> {{ t('tools.ulid-generator.function.quantity')  }}</label>
      <n-input-number v-model:value="amount" min="1" max="100" flex-1 />
    </div>

    <c-buttons-select v-model:value="format" :options="formats" :label="t('tools.ulid-generator.function.format')" label-width="75px" />
    ...
    <div flex justify-center gap-2>
      <c-button data-test-id="refresh" @click="refreshUlids()">
        {{t('tools.ulid-generator.function.refresh')}}
      </c-button>
      <c-button @click="copy()">
        {{t('tools.ulid-generator.function.copy')}}
      </c-button>
    </div>
  </div>
</template>
